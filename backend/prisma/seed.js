...
async function main() {
	...
	// Categories are not in schema, skipping
	...
	const bachelor = await prisma.studentStatus.findUniqueOrThrow({
		where: { name: 'Bachelor' },
	});
	...
	const ts = await prisma.skill.findUniqueOrThrow({
		where: { name: 'TypeScript' },
	});
	const react = await prisma.skill.findUniqueOrThrow({
		where: { name: 'React' },
	});
	await prisma.studentSkill.upsert({
		where: { userId_skillId: { userId: pavel.id, skillId: ts.id } },
		update: {},
		create: { userId: pavel.id, skillId: ts.id },
	});
	await prisma.studentSkill.upsert({
		where: { userId_skillId: { userId: pavel.id, skillId: react.id } },
		update: {},
		create: { userId: pavel.id, skillId: react.id },
	});

	// Categories removed from schema, skipping
	...
	if (!task) {
		task = await prisma.task.create({
			data: {
				...
				description:
					'Need a student helper to structure components and connect a small dashboard to REST API.',
